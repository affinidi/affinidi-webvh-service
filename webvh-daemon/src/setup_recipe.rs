...
pub async fn apply_recipe(
    ...
    print_recipe_banner("webvh-daemon", &recipe);

    let secrets_config = resolve_secrets_config(&recipe, "webvh-daemon-secrets", "webvh");
        ...
        (VtaMode::Online, None) => {
            return Err(AppError::Config(
                "online vta_mode requires --setup-key-file <path>. Run \
                 `webvh-daemon setup --setup-key-out <path>` first."
                    .into(),
            ));
        }
        _ => None,
    ...
    // Daemon uses `webvh-control` template when a mediator is given (so
    // the DID document gets both WebVHHosting + DIDCommMessaging) and
    // `webvh-daemon` template otherwise.
    ...
    let (
        ...
        VtaSetupOutcome::OfflinePreparedOnly(_) => {
            return Err(AppError::Config(
                "vta_mode = \"offline-prepare\" with --from is not yet supported for daemon. \
                 Use `webvh-daemon setup-offline-prepare` for phase 1, then --from with \
                 vta_mode = \"offline-complete\" for phase 2."
                    .into(),
            ));
        }
    };
    ...
    let features = affinidi_webvh_common::server::config::FeaturesConfig {
