              ...
              <Link href={`/dids/${item.mnemonic}`} asChild>
                ...
                >
                  ...
                  {/* Agent names, under the DID rather than above it.
                      ...
                      mnemonic, which is also the link target and the DID's
                      own path segment). Leading with the agent name would
                      stack two friendly labels above the identifier they both
                      point at, and the eye stops trusting which line is the
                      DID. Under the DID it reads as what it is: an alias for
                      the string above it. Same placement as the detail page,
                      so a handle looks the same wherever you meet it. */}
                  {(item.agentNames?.length ?? 0) > 0 && (
                    <View style={styles.agentNamesRow}>
                      <AgentNameChips
                        names={item.agentNames}
                        domain={item.domain}
                        didId={item.didId}
                        size="sm"
                      />
