...
enum Command {
    ...
    Setup {
        ...
        /// non-interactively — see `examples/did-hosting-server-build.toml`.
    ...
    VtaRequest {
        ...
        #[arg(long, default_value = "did-hosting-server")]
        label: String,
        /// Public URL where this server serves DIDs. Bound to the
        /// `did-hosting-daemon` template's `URL` variable so the rendered DID
    ...
    /// import via `did-hosting-server bootstrap-did` / `import-secrets`.
    VtaOpen {
        /// Path to the ASCII-armored sealed bundle.
        #[arg(long)]
        bundle: PathBuf,
        ...
        /// Feed the multibase keys into `did-hosting-server import-secrets`.
    ...
    match cli.command {
        ...
        Some(Command::VtaOpen {
            ...
            if let Err(e) = did_hosting_common::server::vta_setup::run_offline_open_cli(
                ...
                &did_log_out,
                &secrets_out,
                did_hosting_common::server::vta_setup::OfflineOpenNextStep::ImportSecrets {
                    binary: "did-hosting-server",
                },
            ) {
                eprintln!("Error: {e}");
                std::process::exit(1);
...
async fn run_recreate_did(
    ...
    let secrets = secret_store
        .get()
        .await?
        .ok_or("no secrets found — run `did-hosting-server setup` first")?;
...
async fn run_dump_did(
    ...
    use did_hosting_common::did_ops::{content_log_key, content_witness_key, did_key};
...
async fn run_server(config_path: Option<PathBuf>) {
    let config = match AppConfig::load(config_path) {
        ...
        Err(e) => {
            ...
            eprintln!("  did-hosting-server --config <path>");
            eprintln!();
            eprintln!("Or run the setup wizard:");
            eprintln!("  did-hosting-server setup");
            std::process::exit(1);
        }
    };
    ...
    let secrets = match secret_store.get().await {
        ...
        Ok(None) => {
            eprintln!("Error: no secrets found — run `did-hosting-server setup` first");
            std::process::exit(1);
        }
        Err(e) => {
...
async fn run_export_sealed(
    ...
    eprintln!(
        "    did-hosting-server import-sealed --bundle {} \\\n      --expect-digest {} \\\n      --producer-pubkey {producer_pub}",
