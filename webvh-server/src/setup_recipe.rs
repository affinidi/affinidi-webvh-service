...
pub async fn apply_recipe(
    ...
    print_recipe_banner("webvh-server", &recipe);
    ...
    let secrets_config = resolve_secrets_config(&recipe, "webvh-server-secrets", "webvh");
        ...
        (VtaMode::Online, None) => {
            return Err(AppError::Config(
                "online vta_mode requires --setup-key-file <path>. Run \
                 `webvh-server setup --setup-key-out <path>` first to mint and \
    ...
    // `webvh-daemon` template because the server hosts its own DID
    ...
    let (
        ...
        VtaSetupOutcome::SelfManaged(_) => {
            ...
            return Err(AppError::Config(
                "self-managed mode is not supported for webvh-server".into(),
            ));
        }
    };

    let jwt_signing_key = affinidi_webvh_common::server::vta_setup::generate_ed25519_multibase();
