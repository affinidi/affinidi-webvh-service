...
//!
//! Sibling to [`crate::setup`]'s interactive wizard. Pairs with
//! `--from <recipe.toml>` on the `setup` subcommand.

use std::path::{Path, PathBuf};
use std::sync::Arc;

use affinidi_webvh_common::server::operator_messages::WebvhControlMessages;
...
pub async fn run_from_recipe(
    ...
    require_service(&recipe, ServiceKind::Control).map_err(|e| AppError::Config(format!("{e}")))?;
...
pub async fn apply_recipe(
    ...
    print_recipe_banner("webvh-control", &recipe);

    let secrets_config = resolve_secrets_config(&recipe, "webvh-control-secrets", "webvh");
        ...
        (VtaMode::Online, None) => {
            return Err(AppError::Config(
                "online vta_mode requires --setup-key-file <path>. Run \
                 `webvh-control setup --setup-key-out <path>` first."
                    .into(),
            ));
        }
        _ => None,
    ...
    // Online mode mints via `webvh-control` template only when a mediator
    // is configured (the template requires MEDIATOR_DID); otherwise we
    // mint via `webvh-daemon` for HTTP-only and rely on runtime DIDComm
    ...
    let (
        ...
        VtaSetupOutcome::SelfManaged(_) => {
            return Err(AppError::Config(
                "self-managed mode is not supported for webvh-control".into(),
            ));
        }
    };
    ...
    let port = recipe
        .server
        .port
        .unwrap_or_else(|| SetupRecipe::default_port(ServiceKind::Control));
    ...
    // Write the DID log entry file for the operator to publish on the
    // webvh hosting server. Default file name matches the interactive
    // wizard (`control-did.jsonl`).
    ...
    eprintln!();
    eprintln!("  Control DID:       {control_did}");
    eprintln!(
        "  Next: webvh-control --config {}",
        recipe.output.config_path.display()
    );
    eprintln!();
