...
pub fn build_authenticate_body(
    ...
    // Trust-Task envelope. The server deserializes this as
    // `trust_tasks_rs::TrustTask<serde_json::Value>` and only requires
    // `id` + `type` + `payload`; any other top-level members land in
    // its `extra` map. We emit exactly those three so the body stays
