...
pub async fn apply_recipe(
    ...
    print_recipe_banner("did-hosting-control", &recipe);

    let secrets_config = resolve_secrets_config(&recipe, "did-hosting-control-secrets", "webvh");
        ...
        (VtaMode::Online, None) => {
            return Err(AppError::Config(
                "online vta_mode requires --setup-key-file <path>. Run \
                 `did-hosting-control setup --setup-key-out <path>` first."
                    .into(),
            ));
        }
        _ => None,
    ...
    let (
        ...
        VtaSetupOutcome::SelfManaged(_) => {
            return Err(AppError::Config(
                "self-managed mode is not supported for did-hosting-control".into(),
            ));
        }
    };
    ...
    eprintln!(
        "  Next: did-hosting-control --config {}",
        recipe.output.config_path.display()
    );
    eprintln!();
...
async fn persist_offline_prepare(
    ...
    print_offline_prepare_recap("did-hosting-control", recipe, info);
    Ok(())
}
